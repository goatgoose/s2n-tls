...
{
    printf("----------------\n");
    printf("macro: %d\n", s2n_libcrypto_supports_tls_prf());
    printf("----------------\n");
...
int s2n_fuzz_test(const uint8_t *buf, size_t len)
{
    ...
    DEFER_CLEANUP(struct s2n_stuffer fuzz_stuffer = { 0 }, s2n_stuffer_free);
...
//    printf("--------------------------\n");
    ...
    /* Ensure that the custom PRF implementation and the libcrypto PRF implementation produce the
     * same results.
     */
