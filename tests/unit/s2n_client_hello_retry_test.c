...
int main(int argc, char **argv)
{
    ...
    if (s2n_is_tls13_fully_supported()) {
        ...
        /* Test: The server rejects a second ClientHello with a changed session ID */
        {
            DEFER_CLEANUP(struct s2n_connection *server_conn = s2n_connection_new(S2N_SERVER),
                    s2n_connection_ptr_free);
            EXPECT_NOT_NULL(server_conn);
        ...
        /* Test: The server accepts a second ClientHello with a changed cipher suites list */
        ...
        /* Test: The server accepts a second ClientHello with a changed supported versions extension */
        for (uint8_t version = S2N_TLS10; version <= S2N_TLS13; version++) {
            ...
            struct s2n_client_hello *first_client_hello = s2n_connection_get_client_hello(server_conn);
            EXPECT_NOT_NULL(first_client_hello);
            ssize_t first_supported_versions_length = s2n_client_hello_get_extension_length(first_client_hello,
            ...
            EXPECT_SUCCESS(s2n_stuffer_write_uint8(&extension_stuffer, version / 10));
            EXPECT_SUCCESS(s2n_stuffer_write_uint8(&extension_stuffer, version % 10));
            ...
            /* Ensure that the supported versions extension changed in the second ClientHello. */
            ssize_t second_supported_versions_length = s2n_client_hello_get_extension_length(second_client_hello,
                    S2N_EXTENSION_SUPPORTED_VERSIONS);
            ...
            if (version == S2N_TLS13) {
                ...
                /* Ensure that it's not possible to negotiate a protocol version < TLS 1.3 when
                 * receiving the second supported versions extension.
                 */
