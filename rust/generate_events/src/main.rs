...
impl EventInfo<'_> {
    fn s2n_tls() -> Self {
        ...
        impl Default for Subscriber {
            fn default() -> Self {
                let root = tracing::span!(target: "s2n_tls", tracing::Level::DEBUG, "s2n_tls");
        ...
        EventInfo {
            crate_name: "s2n_tls_rust",
            input_path: concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../s2n-tls-events/events/**/*.rs"
            ),
            output_path: concat!(
            env!("CARGO_MANIFEST_DIR"),
...
fn main() -> s2n_quic_events::Result<()> {
    let event_paths = [EventInfo::s2n_tls()];
